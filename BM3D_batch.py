...
if __name__ == '__main__':
    ...
    inputfile='G:/GEE_Data/SelinCo/SAR_Descending/SAR_SelinCo_8bit/'
    outputfile_basic='G:/GEE_Data/SelinCo/SAR_Descending/BM3D_filter_basic/'
    outputfile_final='G:/GEE_Data/SelinCo/SAR_Descending/BM3D_filter_final/'
    imagelist=os.listdir(inputfile)
    for img_name in imagelist:   
        file_count = re.sub("\D", "", img_name)
        filename = "%s.tif" % file_count
        #img_name = "G:/GEE_Data/SelinCo/SAR_SelinCo_8bit/20150215.tif"  # 图像的路径
        img = cv2.imread(inputfile+img_name, cv2.IMREAD_GRAYSCALE)    # 读入图像，cv2.IMREAD_GRAYSCALE:以灰度模式读入图像
    ...
    #     print("success")
        ...
        cv2.imwrite(outputfile_basic+ filename, Basic_img)
    ...
    # Basic_img = cv2.imread("Basic3.jpg", cv2.IMREAD_GRAYSCALE)
        ...
        cv2.imwrite(outputfile_final + filename , Final_img)
